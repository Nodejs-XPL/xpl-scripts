...
class ScriptsHost extends EventEmitter {
	constructor(path, mtime, configuration, query) {
		...
		this._debug = debug('xpl-scripts:scriptsHost:' + Path.basename(path));
	...
	_callScriptFunc(resource, throwException,
		...
		return function () { // pas de () => !!!! (on utilise arguments)
			...
			this._debug("Enter script sandbox  script #", script.id, " type=", resource.type);
			...
			try {
				...
				this._debug("Exit script sandbox #", script.id, " in ", diff, "ms");
			}
		};
	}
