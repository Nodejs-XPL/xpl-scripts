...
class ScriptsHost extends EventEmitter {
	...
	_processMessage(packet, address, buffer) {
		...
		let resources = this._resources.filter((ref) => {
			...
			if (!(ref.script.state & Script.RUNNING)) {
				debug("processMessage", "Trigs #", ref.id, "Not running ", ref.script.state);
				return false;
			}

			if (ref.command && ref.command !== packet.headerName) {
				debug("processMessage", "Trigs #", ref.id, "Not right command", ref.command);
				return false;
			}
			...
			if (refUnit) {
				if (util.isRegExp(refUnit) || util.isArray(refUnit) || typeof(refUnit) === "string") {
					...
					if (refUnit.device && !filter("device", refUnit.device, packet.body.device || packet.body.address)) {
						return false;
					}
		...
		resources.forEach((ref) => {
			debug("processMessage", "Trigs #", ref.id, " hit: call func for script #", ref.script.id);
	...
	_callScriptFunc(resource, throwException,
		...
		return function () { // pas de () => !!!! (on utilise arguments)
			...
			debugEvent("Enter script sandbox  script #", script.id, " type=", resource.type);
