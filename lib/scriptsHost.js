...
class ScriptsHost extends EventEmitter {
	 ...
	 *
	 * @param {Script} script
	 * @param {string|Object|Regexp} unit
	 * @param {Function} func
	 * @returns {*}
	 */
	onXplTrig(script, unit, func) {
		if (typeof(func) !== "function") {
			throw new Error("Invalid func parameter");
		}
	...
	_onXplMessage(cmd, script, unit, func) {
		assert.equal(typeof(cmd), "string", "Invalid cmd parameter");
		assert(script instanceof Script, "Invalid script parameter");
		assert(typeof(unit) === "string" || unit === null || typeof(unit) === "object", "Invalid unit parameter");
		assert.equal(typeof(func), "function", "Invalid func parameter");
	...
	_callScriptFunc(resource, throwException, context) {
