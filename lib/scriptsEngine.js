...
class ScriptEngine extends EventEmitter {
	...
	_sendXpl(event) {
		switch (event.command) {
			case "xpl-cmnd":
				return this.sendXplCmnd(event.body, event.bodyName, event.target, event.source, event.callback);

			case "xpl-stat":
				return this.sendXplStat(event.body, event.bodyName, event.target, event.source, event.callback);

			case "xpl-trig":
				return this.sendXplTrig(event.body, event.bodyName, event.target, event.source, event.callback);

			default:
				return this.sendXplCommand(event.command, event.body, event.bodyName, event.target, event.source, event.callback);
		}
	}

	sendXplCmnd(body, bodyName, target, source, callback) {
		this.xpl.sendXplCmnd(body, bodyName, target, source, callback);
	}

	sendXplStat(body, bodyName, target, source, callback) {
		this.xpl.sendXplStat(body, bodyName, target, source, callback);
	}

	sendXplTrigfunction(body, bodyName, target, source, callback) {
		this.xpl.sendXplTrig(body, bodyName, target, source, callback);
	}
}
