...
var scriptEngineId = 0;

var ScriptEngine = function(name, deviceAliases, xpl) {
  this.id = scriptEngineId++;
  this.xpl = xpl;
...
ScriptEngine.prototype._messageReceived = function(packet, address, buffer) {
  ...
  scriptsHosts.forEach(function(scriptsHost) {
    ...
    scriptsHost.emit("message", packet, address, buffer, deviceAliases);
  });
};
...
ScriptEngine.prototype.newScript = function(name) {
  ...
  console
      .log("Allocate new script '" + name + "' for scriptHost=" + scriptHost);
...
ScriptEngine.prototype._newScriptsHost = function(scriptPath, mtime, callback) {
  var scriptsHost = new ScriptsHost(scriptPath, mtime);
  ...
  setImmediate(function() {
    scriptsHost._initialize(function(error) {
      if (error) {
...
ScriptEngine.prototype._sendXpl = function(event) {
  ...
  var error = new Error("Unknown command '" + event.command + "'");
  error.event = event;
  error.scriptEngine = this;
