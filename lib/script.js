...
class Script extends EventEmitter {
	...
	toString() {
		return "[Script #" + this.id + " name='" + this.name + "' state=" + this.state + "]";
	}
	 ...
	 * @returns {*}
	 */
	sendXplMessage(command, body, bodyName, target, source, callback) {
		...
		return this._scriptHost.sendXplCommand(this, command, body, bodyName, target, source, callback);
	}
	...
	sendXplCmnd(body, bodyName, target, source, callback) {
		...
		return this._scriptHost.sendXplCommand(this, "xpl-cmnd", body, bodyName, target, source, callback);
	}
	...
	sendXplStat(body, bodyName, target, source, callback) {
		...
		source = source || ";" + this.name;
		return this._scriptHost.sendXplCommand(this, "xpl-stat", body, bodyName, target, source, callback);
	}
	...
	sendXplTrig(body, bodyName, target, callback) {
		...
		return this._scriptHost.sendXplCommand(this, "xpl-trig", body, bodyName, target, source, callback);
	}
	...
	queryMinMaxAvgSum(path, options, callback) {
		this._scriptHost._query.getMinMaxAvgSum(path, options, (error, value, response) => {
			if (error) {
				this.error("queryMinMaxAvgSum error for path=", path, "error=", error);
			}
